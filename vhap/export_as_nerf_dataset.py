...
class NeRFDatasetWriter:
    def __init__(
        ...
        pass  # config logging removed for production perf
...
class MaskFromFLAME:
    def __init__(
        self, cfg_model: ModelConfig, tgt_folder, background_color: str
    ) -> None:
        background_color = (
            self.cfg_data.background_color
        ...
        self.dataloader = DataLoader(
            dataset, shuffle=False, batch_size=None, collate_fn=None, num_workers=0
        )
    ...
    def get_mask(self, verts, RT, K, h, w):
        ...
        out_dict = self.mesh_renderer.render_without_texture(
            verts, faces, RT, K, (h, w)
        )
