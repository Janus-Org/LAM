...
def expand_bbox(bbox, scale=1.1):
    ...
    return torch.stack(
        [x_min_expanded, y_min_expanded, x_max_expanded, y_max_expanded], dim=-1
    )
...
def load_config(src_folder: Path):
    ...
    if not config_file_path.exists():
        src_folder = sorted(src_folder.iterdir())[-1]  # Get the last modified folder
        config_file_path = src_folder / "config.yml"
...
class FlameTrackingSingleImage:
    ...
    def __init__(
        ...
        assert os.path.exists(alignment_model_path), (
            f"{alignment_model_path} does not exist!"
        )
        if args is None:
        ...
        self.alignment = Alignment(
            args, alignment_model_path, dl_framework="pytorch", device_ids=[0]
        )

        # Load VGG head model
        assert os.path.exists(vgghead_model_path), (
            f"{vgghead_model_path} does not exist!"
        )
        ...
        assert os.path.exists(facebox_model_path), (
            f"{facebox_model_path} does not exist!"
        )
        self.detector = FaceBoxesDetector(
            "FaceBoxes", facebox_model_path, True, self.device
        )
        ...
        logger.info(
            f"Finished Loading Pre-trained Models. Time: {end_time - start_time:.2f}s"
        )
    ...
    def _parse_args(self):
        ...
        parser.add_argument(
            "--config_name", type=str, help="Configuration name", default="alignment"
        )
        parser.add_argument("--blender_path", type=str, help="Blender path")
    ...
    def preprocess(self, input_image_path):
        ...
        # Apply matting
        cropped_frame, mask = self.matting_engine(
            cropped_frame / 255.0, return_type="matting", background_rgb=1.0
        )
        cropped_frame = cropped_frame.cpu() * 255.0
        saved_image = np.round(cropped_frame.cpu().permute(1, 2, 0).numpy()).astype(
            np.uint8
        )[:, :, (2, 1, 0)]
        ...
        cv2.imwrite(os.path.join(output_image_dir, name_list[frame_index]), saved_image)
        cv2.imwrite(
            os.path.join(output_mask_dir, name_list[frame_index]),
            np.array((mask.cpu() * 255.0)).astype(np.uint8),
        )
        cv2.imwrite(
            os.path.join(output_alpha_map_dir, name_list[frame_index]).replace(
                ".png", ".jpg"
            ),
        ...
        for idx, detection in enumerate(detections):
            ...
            try:
                face_landmarks = self.alignment.analyze(
                    saved_image, scale, center_w, center_h
                )
            except Exception:
                logger.warning("Failed to estimate landmarks for one detection")
                continue

            if face_landmarks is not None and face_landmarks.shape[0] > 0:
                break
        ...
        end_time = time.time()
        torch.cuda.empty_cache()
        logger.info(f"Finished Processing Image. Time: {end_time - start_time:.2f}s")
    ...
    def optimize(self):
        ...
        config_data.data.root_folder = Path(os.path.dirname(self.sub_output_dir))
        ...
        end_time = time.time()
        torch.cuda.empty_cache()
        logger.info(f"Finished Optimization. Time: {end_time - start_time:.2f}s")
    ...
    def _detect_iris_landmarks(self, image_path):
        ...
        if len(face_detections) != 1:
            ...
            json.dump(
                landmark_data,
                open(os.path.join(self.sub_output_dir, "landmark2d", "iris.json"), "w"),
            )
    ...
    def export(self):
        ...
        tgt_folder = Path(self.output_export, self.sub_output_dir.split("/")[-1])
        src_folder, config_data = load_config(src_folder)

        nerf_writer = NeRFDatasetWriter(
            config_data.data, tgt_folder, None, None, "white"
        )
        nerf_writer.write()

        flame_writer = TrackedFLAMEDatasetWriter(
            config_data.model, src_folder, tgt_folder, mode="param", epoch=-1
        )
        flame_writer.write()
